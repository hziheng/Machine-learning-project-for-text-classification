...
class XSoftmax(torch.autograd.Function):
    ...
    def symbolic(g, self, mask, dim):
        ...
        r_mask = g.op(
            "Cast",
            g.op("Sub", g.op("Constant", value_t=torch.tensor(
                1, dtype=torch.int64)), mask_cast_value),
            to_i=sym_help.cast_pytorch_to_onnx["Byte"],
        )
        output = masked_fill(
            g, self, r_mask, g.op("Constant", value_t=torch.tensor(
                torch.finfo(self.type().dtype()).min))
        )
...
def make_log_bucket_position(relative_pos, bucket_size, max_position):
    ...
    log_pos = (
            torch.ceil(torch.log(abs_pos / mid) /
                       torch.log(torch.tensor((max_position - 1) / mid)) * (mid - 1)) + mid
    )
...
class DisentangledSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if "p2c" in self.pos_att_type:
            scale_factor += 1
        scale = torch.sqrt(torch.tensor(query_layer.size(-1),
                                        dtype=torch.float) * scale_factor)
    ...
    def disentangled_attention_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if "c2p" in self.pos_att_type:
            scale = torch.sqrt(torch.tensor(
                pos_key_layer.size(-1), dtype=torch.float) * scale_factor)
        ...
        if "p2c" in self.pos_att_type:
            scale = torch.sqrt(torch.tensor(
                pos_query_layer.size(-1), dtype=torch.float) * scale_factor)
...
class bert_classify(DebertaV2PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    ...
    def __init__(self, config):
        ...
        self.bilstm = nn.LSTM(config.hidden_size, 100, bidirectional=True)
        self.classifier = nn.Linear(config.hidden_size + 200, self.num_labels)
        # 随机初始化只对最后增加的线性层，已加载的模型参数不受影响
        self.init_weights()
        self.post_init()
    ...
    def forward(
        ...
        hidden_layers_last = outputs[0]
        rnn_output, _ = self.bilstm(hidden_layers_last)  # shape [batchsize, 200]
        ...
        cls_output = torch.cat((rnn_output[:, -1], cls_output), dim=1)
